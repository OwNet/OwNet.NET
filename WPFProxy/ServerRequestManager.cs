...
namespace WPFProxy
{
    public class ServerRequestManager
    {
        ...
        private static HttpClient CreateClient() {
            HttpClient ret = new HttpClient(Controller.ServiceBaseUrl);
            ret.TransportSettings.ConnectionTimeout = new TimeSpan(0, 1, 0);   // 10 seconds timeout for Client
            return ret;
        }

        public static T Post<P, T>(string url, P data)
        {
            T ret = default(T);
        ...
        public static T Get<T>(Uri uri, HttpQueryString vars = null)
        {
            T ret = default(T);
            using (HttpClient http = CreateClient())
            {
